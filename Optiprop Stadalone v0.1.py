...
for i in range(len(X)*2):
    i = i - len(X)
    Oval.append(    [-abs(math.cos(math.radians(Alpha*i))*(Achse/2+InnerRingBreite*1.2))+(Achse/2)+InnerRingBreite*2+xO,   (math.cos(math.radians(Alpha*i)))*math.sqrt((Achse/2+InnerRingBreite+RingbreiteAussen)**2-1.6**2)+yO,     math.sin(math.radians(Alpha*i))*3+zO])     
...
for i in range(len(X)*4):
    if i % 2 == 0:
        ...
        Eckfolge.append([0+f+(len(X)*2),1+f+(len(X)*2),1+f])
        f += 1
Eckfolge.append([1,121,122])


Hubmesh = mesh.Mesh(np.zeros(np.array(Eckfolge).shape[0], dtype=mesh.Mesh.dtype))
for i, f in enumerate(Eckfolge):
    for j in range(3):
        Hubmesh.vectors[i][j] = np.array(Oval)[f[j],:]    
Hub.append(Hubmesh.data)  
